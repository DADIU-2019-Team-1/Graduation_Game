...
//[ExecuteInEditMode]
    public class Debugger : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                Debug.LogError(
                    "Debugger Error: MotionMatching script not found. Please have this script attached to an object with a MotionMatching script attached");
                runOnce = false;
            }
    ...
    public class VersionDisplay_Editor : UnityEditor.Editor
    {
            ...
            if (script.drawSelectedPos)
            {
                Handles.BeginGUI();
                {
                    GUI.Label(new Rect(0, 0, 600, 30), "Debugger active showing "
                                                       + script.selectJoints + " joints over " +
                                                       script.GetJointsAmount() + " frames", handleStyle);
                }
            }
        }
    }
